...
class MPCHandler: NSObject, MCSessionDelegate {
    ...
    func setupBrowser(){
        browser = MCBrowserViewController(serviceType: "awquaint", session: session)
    }
    
    func advertiseSelf(advertise:Bool){
        if advertise{
            advertiser = MCAdvertiserAssistant(serviceType: "awquaint", discoveryInfo: nil, session: session)
            advertiser!.start()
        }else{
            advertiser!.stop()
            advertiser = nil
